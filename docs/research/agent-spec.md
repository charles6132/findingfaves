...
**Status:** debugger and implementer are **written and committed**. The analyzer is **specified but not built**. Hooks are written but **never executed** — see §7.
...
│   └── analyzer.md          ❌ specified below, not written
├── hooks/
│   ├── verify-edit.sh       ✅ written, ⚠️ never run
│   └── verify-done.sh       ✅ written, ⚠️ never run
...
## 5. `analyzer` — specified, NOT built

Intended for "explain this codebase / assess this subsystem" work, where the failure mode is *confident summary of code the agent never opened*.
...
**Output contract to implement:** Purpose / Structure (entry points, key modules, data flow) / Dependencies / Risk areas (complexity, missing tests, sharp edges) / Open questions.

**Prompt rules to include:**
- Cite `file:line` for every structural claim — no unsourced assertions
- Distinguish "I read this" from "I inferred this"
...
## 7. Hooks — written, NEVER EXECUTED

⚠️ **Both hook scripts pass `bash -n` syntax checking only. Neither has ever run against a real edit or a real test suite. Treat them as drafts.**
...
**Before trusting these, test them:** make a trivial edit in a project with a known-failing test and confirm (a) `verify-edit` surfaces lint output, (b) `verify-done` blocks exactly once and then permits the stop.
...
1. **Execute the hooks against a real project.** They are unvalidated. Highest risk item here.
2. **Redo the lost research leg** — published production system prompts and process patterns (see findings §7, `UNRECOVERABLE`). Use the 3-stage design so it does not cost 26M tokens.
3. **Build `analyzer`** from §5.
4. **Attach `mcp-debugger`** to `debugger` via `mcpServers` — the +11–15 pp lever.
5. **Build an eval set** from this repo's own merged PRs (~50), oracle hidden, run weekly. This is how you find out whether any of the above actually helps.
6. **Restructure the deep-research skill** per findings §5 — `maxTurns` caps, incremental note-writing, staged execution.
