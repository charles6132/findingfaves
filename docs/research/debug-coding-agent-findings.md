...
- **Most of this has still not been validated by running it.** The staged research design and the 85–90% cost estimate remain untested, and the three agent definitions in `.claude/agents/` have not been measured against real work — whether these prompts help *this* codebase is still a guess until the eval set exists.
- **The hooks are the exception, as of 2026-09-18.** Both were executed against a purpose-built project and behaved as designed, after two real bugs in the Go branch were found and fixed. See `agent-spec.md` §7. This is a small but real datapoint for the document's own thesis: both bugs passed `bash -n` and were invisible to reading. Constraints you have not executed are not constraints — the same failure mode as a budget written in prose.
