...
1. **Depth defaults to quick.** Quick = 3 sub-questions, 1 round, no gap-fill.
   ...
   request.
2. **Perspective mining** (STORM): before writing questions, skim 2-3 overview
   sources (Wikipedia, a recent survey, the topic's main hub) and list 3-5
   distinct perspectives on the topic. Direct prompting produces shallow
   questions; perspectives force coverage of angles you would not think of.
3. Decompose the topic into 3-8 sub-questions, one per perspective, each
   independently researchable. Each must be answerable from sources, not opinion.
4. Set the budget and write it into `plan.md`:
...
5. Save `plan.md`. This is the first checkpoint — a crash here resumes from the
   plan, not from scratch.

## Phase 1 — Parallel research

Launch one subagent per sub-question with the Task tool, all in a single message
(parallel, output disabled — each writes its own file).

Each researcher prompt must contain:

- The sub-question and its perspective.
- The time range.
- **Engine recipes**: read `~/.claude/skills/research-engines/ENGINES.md`
  and follow it. Search **only** through `research-engines/search.ps1` — never
...
- **Work in batches, not one call at a time.** This is the single biggest cost
  ...
  1. **Search — all at once.** Write every query you need up front, then fire
     them in a single message as parallel calls. Do not search, read, think,
     search again. Five queries in one turn cost one context read; five queries
     in five turns cost five.
  2. **Fetch — all at once.** Pick the URLs worth reading from all the search
     results together, then fetch them in one parallel batch.
  ...
  4. **Write the evidence JSON.** No further searching.
...
```
python "C:\Users\Charles\.claude\skills\research-engines\merge_evidence.py" "{topic_slug}"
```
...
1. **URL liveness**: check every URL in the ledger in ONE call —
   ...
   ```
   powershell -NoProfile -File "C:\Users\Charles\.claude\skills\research-engines\checkurls.ps1" "{topic_slug}/ledger.json"
   ```
